...
def format_newsletter(newsletter):
    ...
    for category, data in newsletter.items():
        ...
        if data["papers"]:
            for i, paper in enumerate(data["papers"], 1):
                # mailto: link pre-fills an email to yourself with the title — works in all email clients
                mailto_subject = url_quote(f"REJECT: {paper['title']}")
                mailto_body = url_quote(paper['title'])
                ...
                <div class="paper">
                    ...
                    <a class="reject-link" href="{mailto_href}" title="Send yourself an email with this title to reject it">[Reject]</a>
                </div>
                """
        else:
    ...
    html += """
        <div class="footer">
            ...
            <em>Click [Reject] to email yourself the title, then paste it into feedback.json &rarr; rejected_titles</em></p>
        </div>
        </div>
    </body>
    </html>
