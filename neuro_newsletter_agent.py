...
def format_newsletter(newsletter):
    ...
    for category, data in newsletter.items():
        ...
        if data["papers"]:
            for i, paper in enumerate(data["papers"], 1):
                # mailto: opens email with reject command in body — copy and run in terminal
                reject_cmd = f'python neuro_newsletter_agent_1.py --reject "{paper["title"]}"'
                mailto_subject = url_quote("REJECT paper")
                mailto_body = url_quote(reject_cmd)
                mailto_href = f"mailto:{EMAIL_CONFIG['recipient']}?subject={mailto_subject}&body={mailto_body}"
                html += f"""
                <div class="paper">
                    ...
                    <a class="reject-link" href="{mailto_href}" title="Email yourself the reject command">[Reject]</a>
                </div>
                """
        else:
    ...
    html += """
        <div class="footer">
            ...
            <em>Click [Reject] to email yourself the command. Then run it in terminal to safely add to feedback.json</em></p>
        </div>
        </div>
    </body>
    </html>
